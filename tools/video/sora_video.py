...
The SDK's create() always sends input_reference as a multipart file, so
an image_url reference cannot be passed in the JSON body.
This calls the REST endpoint directly to work around that.

Downloads use the /videos/{id}/content REST endpoint.
"""
...
class SoraVideo(BaseTool):
    ...
    def _download_and_return(self, video_id: str, output_path: str, start: float) -> ToolResult:
        """Save the mp4 from the REST /videos/{id}/content endpoint."""
        resp = requests.get(
